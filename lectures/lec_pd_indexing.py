#overview
'''pandas indexing
Pandas stores data in series and data frames.
Pandas provides several ways of selecting and slicing data.
most commonly used approaches: "basic" [] indexing (like with dictionaries) and two methods, .loc, .iloc.
.loc: Indexing only via labels (label-based indexing)
.iloc: Indexing only via integer position (positional indexing)
[]: Combines both label-based and positional indexing.
/Selection                         | Result       | Notes                                     |
|-----------------------------------|--------------|-------------------------------------------|
| Series.loc[label]                 | scalar value | Label must exist, KeyError otherwise      |
| Series.loc[list of labels]        | Series       | All labels must exist, KeyError otherwise |
| Series.loc[start_label:end_label] | Series       | Behaviour varies
'''
...
# df.loc[row indexer, col indexer] end element will included
# For instance, selecting the close price on January 3, 2020
x = df.loc['2020-01-03', 'Close']
print(x)  # --> 7.19   only one value
...
y = df.loc['2020-01-03', :]  #Close    7.19
#                             Bday     2.00
...
df2.rename(index={'2020-01-08':'1900-01-01'}, inplace=True)  # implace = True作用是？
# sort function basically create a copy, if want to modify the original dataframe, need inplace=True
print(df2)

x = df2.loc['2020-01-03':'2020-01-10', :]
print(x)
'''IN-CLASS-EXERCISE1
please select teh values for the row indexes['b','c', 'd', 'e'] fro col1 from the following dataframe
make sure the data is returned as a !!!Series!!!
df = pd.DataFrame(
    {
        'coll': range(10),
        'col2': range(10, 20)
...
#sort the row indexes in-place
df.sort_index(inplace=True)
#select the data using loc
result = df.loc['b':'e','col1']
print(result)
'''in-class-exercise2
select the values for the row index['b', 'd', 'f', 'j'] fro col1 from the above dataframe
however, make sure you return the data in a dataframe'''
df = pd.DataFrame(
    {
        'col1': range(10),
        'col2': range(10, 20)
...
result = df.loc[['b', 'd', 'f', 'j'], ['col1']]  # <--'col1' is surrounded by []
print(result)
# result:
#   col1
#b     6
...
#   (only works with positions) when using .iloc endpoints are not included!
# ----------------------------------------------------------------------------
...
x = ser.iloc[0]  # the first one element
x = ser.iloc[-1]  # the last one element

#x = ser.iloc[100] # raises IndexError, out of series
...
# Equivalent to
x = df.iloc[0,:] # all the col for the row
print(x)
...
x = df.iloc[:,0]  # all the rows from the first col
print(x)
...
# bounds:
#x = df.iloc[[0, 10]] # --> raises IndexError, only have 10, but this is 11
...
x = df.iloc[1:1000, :1]  # second row to the end
print(x)
...
'''IN-CALSS-exercise3
please select the values for the even-indexed rows col2 from the following dataframe
df = pd.DataFrame(
    {
        'col1':range(0, 20),
        'col2':range(20, 40),
...
rows= list(range(0, len(df), 2))  # get the even row indexes of the dataframe
...
# ----------------------------------------------------------------------------
#   3. Selection using []
# using index labels(label)
# using the position of the element in the array(pos)
# ----------------------------------------------------------------------------
